...
class AddStudent():
    def __init__(self,root):
        ...
        button2=ttk.Button(menu_frame,text='Add to Data Base',width=20,command=self.add_student_details).grid(row=0,column=2,padx=4,pady=3)
        ...
        #self.cource_frame=Frame(self.root,bd=2,relief=RIDGE)
        #self.cource_frame.place(x=600,y=100,width=530,height=270)
        ...
        frame2=ttk.Frame(addStudentNoteBook,width=1035,height=260)
        frame3=ttk.Frame(addStudentNoteBook,width=1035,height=260)
        frame4=ttk.Frame(addStudentNoteBook,width=1035,height=260)
        ...
        addStudentNoteBook.add(frame2,text='      All Student Details     ')
        addStudentNoteBook.add(frame3,text='     Additional Details of Student   ')
        addStudentNoteBook.add(frame4,text='     Dara Base of Additional Details of Student   ')
        ...
        def select_student_image():
            image=openfilename()
            student_image=Image.open(image)
            student_image=student_image.resize((250,250))
            student_image=ImageTk.PhotoImage(student_image)
            panel=tk.Label(frame1,image=student_image,relief=GROOVE,width=290,height=150)
            panel.image=student_image
            panel.place(x=730,y=95)
            
            
        def openfilename():
            filename=filedialog.askopenfilename(title='open',parent=self.root)
            return filename
        ...
        #select_image_path=ttk.Entry(frame1,width=48,textvariable=self.var_student_image_path)
        #select_image_path.place(x=730,y=59)
        dumy_image_label=tk.Label(frame1,relief=GROOVE,width=42,height=10)
        dumy_image_label.place(x=730,y=95)

        self.student_table_frame=Frame(frame2,bd=2,relief=FLAT)
        self.student_table_frame.place(x=5,y=5,width=1025,height=250)
        ...
        self.student_table.heading('student_image',text='Student Image')
...
##################################################frame 3 starts
        ...
        ########################################################
        self.treeview_frame=Frame(frame4,bg='black')
        self.treeview_frame.place(x=10,y=10,width=1000,height=250)
        #########################################################
    ...
    ################BUTTON
        #student_srch_label=tk.Label(frame3,text='Serch Deatails  :',font=("times new roman",13,'bold')).place(x=700,y=15)     
        #student_roll_number_entry_from_database=ttk.Entry(frame3,textvariable=self.var_student_roll_number_from_database,width=30)
        #student_roll_number_entry_from_database.place(x=830,y=15)
    ...
    def fetch_student_personal_data(self):
        ...
        try:
            ...
            rows=cur.fetchall()
            self.student_personal_table.delete(*self.student_personal_table.get_children())
            #self.student_personal_table('oddrow',background='lightblue')
           
            for row in rows:
                #self.student_personal_table.insert(' ',END,values=row)
                self.student_personal_table.insert('',END,values=row)
                #self.student_personal_table.tag_configure('odd', background='#E8E8E8')
